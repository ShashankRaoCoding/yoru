...
func runImage(args []string) error {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: yoru sixel print image -i <format> [width] [height]\n")
		...
		fmt.Fprintf(os.Stderr, "\nExamples:\n")
		fmt.Fprintf(os.Stderr, "  cat image.png | yoru sixel print image -i png\n")
		fmt.Fprintf(os.Stderr, "  cat image.jpg | yoru sixel print image -i jpeg 100 50\n")
	}
	...
	targetWidth, targetHeight, err := parseDimensions(fs.Args())
	if err != nil {
		return err
	}

	format := normalizeImageFormat(*inputFormat)
	...
	_, err = fmt.Fprint(os.Stdout, encode(img))
	return err
}
...
func parseDimensions(remaining []string) (int, int, error) {
	var err error
	...
	if len(remaining) >= 1 {
		...
		if err != nil {
			return 0, 0, fmt.Errorf("invalid width %q: %w", remaining[0], err)
		}
	}
	if len(remaining) >= 2 {
		targetHeight, err = strconv.Atoi(remaining[1])
		if err != nil {
			return 0, 0, fmt.Errorf("invalid height %q: %w", remaining[1], err)
		}
	}
	if len(remaining) > 2 {
		return 0, 0, fmt.Errorf("unexpected extra arguments after height")
	}
