...
func Main(args []string) {
	...
	fmt.Println("\nSQL Command Usage:")
	fmt.Println("  yoru sql [options] <query> [dbpath]")
	fmt.Println("\nSQL Options:")
	fmt.Println("  -i format   Input format: csv, tsv, or sqldb (default: csv)")
	fmt.Println("  -o format   Output format: csv, tsv, or sqldb (default: csv)")
	fmt.Println("\nSQL Examples:")
	...
	fmt.Println("  yoru sql -i sqldb -o csv 'SELECT * FROM table' input.db")
	fmt.Println("  cat data.csv | yoru sql -i csv -o sqldb 'SELECT * FROM table' output.db")
}
